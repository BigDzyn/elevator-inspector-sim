...
const pdfCanvasContext = elements.pdfCanvas.getContext("2d");
...
function renderQuestion() {
  ...
  question.choices.forEach((choice, choiceIndex) => {
    ...
    if (question.userAnswer === choiceIndex) {
  ...
  elements.nextQuestion.textContent =
    state.currentIndex === state.questions.length - 1 ? "Review Exam" : "Next";
}
...
function renderReviewScreen() {
  ...
  state.questions.forEach((question, index) => {
    ...
    if (question.flagged) {
      badges.push('<span class="badge warn">Flagged</span>');
    }
...
function buildCategoryBreakdown() {
  ...
  state.questions.forEach((question) => {
    if (!categories.has(question.category)) {
      categories.set(question.category, { total: 0, correct: 0 });
    }
    ...
    card.innerHTML = `
      ...
      ${
        state.showExplanations
          ? `<div class="explanation"><strong>Why:</strong> ${question.explanation}</div>`
          : ""
      }
    `;
...
function updatePdfControls() {
  ...
  elements.pdfPageIndicator.textContent = `Page ${state.pdfPageNumber} / ${state.pdfPageCount || 0}`;
  elements.pdfPrevPage.disabled = !state.pdfDoc || state.pdfPageNumber <= 1;
  elements.pdfNextPage.disabled = !state.pdfDoc || state.pdfPageNumber >= state.pdfPageCount;
...
async function renderPdfPage() {
  ...
  try {
    setPdfLoading(true, `Loading page ${state.pdfPageNumber}...`);

    if (state.pdfRenderTask) {
      state.pdfRenderTask.cancel();
    }

    const page = await state.pdfDoc.getPage(state.pdfPageNumber);
    ...
    if (error?.name !== "RenderingCancelledException") {
      elements.pdfSearchStatus.textContent = "The current page could not be rendered.";
    }
  } finally {
...
async function loadPdfDocument(path) {
  try {
    ...
    elements.pdfSearchStatus.textContent =
      "This reference could not be loaded in the embedded viewer. It may block in-browser rendering or require a different source.";
    setPdfLoading(false);
    updatePdfControls();
  }
}
...
async function runPdfSearch() {
  ...
  try {
    ...
    for (let pageNumber = 1; pageNumber <= state.pdfDoc.numPages; pageNumber += 1) {
      ...
      if (pageText.includes(normalizedKeyword)) {
    ...
    state.pdfPageNumber = matches[0];
    await renderPdfPage();
    elements.pdfSearchStatus.textContent = `Found ${matches.length} page match${matches.length === 1 ? "" : "es"} for "${keyword}". Showing page ${matches[0]}.`;
  } catch (error) {
    elements.pdfSearchStatus.textContent =
      "This reference could not be searched. Some scanned or protected PDFs may not expose searchable text.";
  } finally {
    setPdfLoading(false);
  }
}
...
function changePdfPage(direction) {
  ...
  const nextPage = state.pdfPageNumber + direction;

  if (nextPage < 1 || nextPage > state.pdfPageCount) {
    return;
  }
...
elements.pauseExam.addEventListener("click", () => {
  togglePauseExam();
});
elements.resumeExam.addEventListener("click", () => {
  togglePauseExam(false);
});
...
window.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && elements.pdfModal.classList.contains("active")) {
    closePdfModal();
  }
});
